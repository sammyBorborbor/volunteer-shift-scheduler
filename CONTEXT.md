...
| No shift edit/cancel (FR-10) | Should-have, deprioritized in favor of FR-09/FR-11 (safe to build quickly); FR-10 is not — `signups.shift_id` has no `on delete cascade` against `shifts` (verified live: deleting a shift with any signups fails with a `23503` foreign-key violation), so a hard-delete needs a real decision (soft-delete column, or explicit signup-cascade behavior), not a quick UI addition | A coordinator who makes a mistake on a shift (wrong date, wrong capacity) has to live with it or manually fix it in Supabase | Medium | Add an edit form to `CreateShift`-adjacent route; for cancel, decide soft-delete vs. cascading the existing signups to a `cancelled`/notified state before allowing a hard delete |
