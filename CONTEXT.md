...
| Coordinator has no view of their own cancelled shifts | Cancelling (FR-10, migration `0010`) filters `cancelled_at is null` out of `get_upcoming_shifts_with_capacity`, which both the coordinator dashboard and volunteer browse list read from | A coordinator can't see a list of shifts they've cancelled — only reachable if they still have the roster URL, or via an affected volunteer's own history | Low | Add a "Cancelled" filter/section to `CoordinatorHome`, reading shifts with `cancelled_at is not null` directly (bypassing the RPC, same as `useRoster` already does) |
